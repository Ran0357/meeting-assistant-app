...
const UploadScreen: React.FC<UploadScreenProps> = ({ onMinutesGenerated, onError, onBack }) => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const minutes = await generateMinutesFromText(fileContent);
  ...
  return (
    <div className="w-full flex flex-col items-center text-center">
      ...
      <p className="text-slate-600 mb-8">ZoomやGoogle Meetからエクスポートした文字起こしテキストファイル（.txt）をアップロードしてください。</p>
      ...
      <div className="mt-8 flex items-center space-x-4">
        ...
        <button
          ...
          className="px-8 py-3 bg-green-500 text-white rounded-full text-lg font-semibold shadow-lg hover:bg-green-600 disabled:bg-slate-400 disabled:cursor-not-allowed transition-colors flex items-center"
        >
